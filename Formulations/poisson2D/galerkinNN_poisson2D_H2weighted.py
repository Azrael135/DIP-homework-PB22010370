...
class GNN:
    ...
    on H2 (i.e. X = H2_beta, V = L2_beta, W = H^{3/2}_beta).
    ...
    """
    def __init__(self, neurons: int, scale: np.float64, Lx: np.float64, Ly: np.float64, isExtended: bool, 
                 ...
                 boundaryPen1: np.float64, boundaryPen2: np.float64, 
                 scale_increment: np.float64, base_lr: np.float64, iter_decay_rate: np.float64, 
                 exp_decay_rate: np.float64, maxEpoch: int, maxRef: int, RESULTS_PATH: str, beta: np.float64):
        ...
        self.intDerivatives["u"] = ["00", "11"] 
        self.bdryDerivatives["u"] = ["0", "1"] 
    ...
    def boxInit(self, m: int, n: int, flag: int, ki: int, L: int, ell: int, numWaves: int):
        ...
        if (m == 2 * self.QuadRules.interior_x.shape[1] * numWaves):
            # llcorners = np.array([[-1, -1], [-1, 0], [0, 0]])

            # corner_indices = np.random.choice(len(llcorners), size=n)
            # unit_coords = np.random.random(size=(n, 2))

            # p_tmp = unit_coords + llcorners[corner_indices]
            # p = p_tmp[0:n,:]

            # p = np.array([(random.uniform(-np.pi, np.pi), random.uniform(-1.0, 1.0)) for _ in range(n)])
            # p = np.reshape(p, [n, m])    

            # p = np.array([(random.uniform(0.0, 1.0), random.uniform(0.0, 1.0)) for _ in range(n)])
            # p = np.reshape(p, [n, m])
    ...
    def createNetwork(self, X, Y, params, Cnn, scale, numWaves, alphaSize=None, Calpha=None):
        """
        ...
        """
        # first hidden layer
        W, b, Wr, scale_NN, wxm, wym = params[0]
        ...
        for idx, (W, b, Wr, scale_NN, _, _) in enumerate(params[1:]):
            if (idx == len(params)-2):
        ...
        if (self.isExtendedNetwork):
            ...
            R = jnp.sqrt(jnp.pow(X, 2) + jnp.pow(Y, 2))
            T = jnp.mod(jnp.arctan2(Y, X) + np.pi * 2.0, 2.0 * np.pi)
            R = R.reshape(-1, 1)
            T = T.reshape(-1, 1)
            Ualpha = jnp.multiply(jnp.power(R, alpha), jnp.sin(jnp.multiply(T, alpha))) @ Calpha
            U = Unn + Ualpha
        else:
            U = Unn
    ...
    def networkArrayReg(self, X, Y, params, scale, numWaves, alphaSize=None):
        """
        ...
        """
        W, b, Wr, scale_NN, wxm, wym = params[0]
        Wx = jnp.reshape(W[0,:], [1, W.shape[1]])
        ...
        if (self.isExtendedNetwork):
            ...
            R = jnp.sqrt(jnp.pow(X, 2) + jnp.pow(Y, 2))
            T = jnp.mod(jnp.arctan2(Y, X) + np.pi * 2.0, 2.0 * np.pi)
            Hnn = jnp.column_stack((Hnn, jnp.multiply(jnp.power(R, alpha), jnp.sin(jnp.multiply(T, alpha)))))
    ...
    def activMatrix(self, i, k, NN, solution0,
        ...
        if (self.isExtendedNetwork):
            w = np.sqrt(xTrain**2 + yTrain**2)**self.beta
            # wBdry = np.sqrt(xBdry**2 + yBdry**2)**self.beta
            wBdry = 1.0
        else:
            w = np.sqrt(xTrain**2 + yTrain**2)**self.beta
            wBdry = 1.0
        ...
        NNdy2phi_idx = np.reshape(NN["u"]["interior_yy"][:,i], [self.QuadRules.interior_x.shape[0], 1])
        NNdy2phi_jdx = NN["u"]["interior_yy"]

        A[i,:] = np.sum(self.QuadRules.interior_w * (w*(NNdx2phi_idx + NNdy2phi_idx)) * (w*(NNdx2phi_jdx + NNdy2phi_jdx)), axis=0)
        ...
        NNdxbdryphi_idx = np.reshape(NN["u"]["boundary_x"][:,i], [self.QuadRules.boundary_x.shape[0], 1])
        NNdxbdryphi_jdx = NN["u"]["boundary_x"]
        
        NNdybdryphi_idx = np.reshape(NN["u"]["boundary_y"][:,i], [self.QuadRules.boundary_x.shape[0], 1])
        NNdybdryphi_jdx = NN["u"]["boundary_y"]
        
        A[i,:] += self.boundaryPen2 * np.sum(self.QuadRules.boundary_w * (wBdry*(self.QuadRules.boundary_t_x * NNdxbdryphi_idx + self.QuadRules.boundary_t_y * NNdybdryphi_idx)) * 
                                                                         (wBdry*(self.QuadRules.boundary_t_x * NNdxbdryphi_jdx + self.QuadRules.boundary_t_y * NNdybdryphi_jdx)), axis=0)

        # RHS
        F[i] = np.sum(self.QuadRules.interior_w * (w*DATA["F1"]) * (w*(-NNdx2phi_idx - NNdy2phi_idx)))
        F[i] += self.boundaryPen1 * np.sum(self.QuadRules.boundary_w * (wBdry*NNbdryphi_idx) * (wBdry*DATA["G1"]))
        F[i] += self.boundaryPen2 * np.sum(self.QuadRules.boundary_w * (wBdry*(self.QuadRules.boundary_t_x * NNdxbdryphi_idx + self.QuadRules.boundary_t_y * NNdybdryphi_idx)) *
                                                                       (wBdry*(self.QuadRules.boundary_t_x * DATA["G1x"] + self.QuadRules.boundary_t_y * DATA["G1y"])))
        F[i] += -np.sum(self.QuadRules.interior_w * (w*(NNdx2phi_idx + NNdy2phi_idx)) * 
                                                    (w*(solution0["u"]["interior_xx"] + solution0["u"]["interior_yy"])))
        F[i] += -self.boundaryPen1 * np.sum(self.QuadRules.boundary_w * (wBdry*NNbdryphi_idx) * (wBdry*solution0["u"]["boundary_value"]))
        F[i] += -self.boundaryPen2 * np.sum(self.QuadRules.boundary_w * (wBdry*(self.QuadRules.boundary_t_x * NNdxbdryphi_idx + self.QuadRules.boundary_t_y * NNdybdryphi_idx)) *
                                                                        (wBdry*(self.QuadRules.boundary_t_x * solution0["u"]["boundary_x"] + self.QuadRules.boundary_t_y * solution0["u"]["boundary_y"])))
    ...
    def galerkinUpdate(self, k, neurons, params, solution0, scale, numWaves, alphaSize, DATA):
        ...
        for field in self.solution_fields:
            ...
            NNoutput[field]["interior_value"] = jnp.squeeze(jax.vmap(self.networkArrayReg, (0, 0, None, None, None, None),  0)
                                                            (xTrain, yTrain, params_i[field], scale, numWaves, opt_args))
            for der in self.intDerivatives[field]:	
                # compute the requested derivative
                der_iter = 0
                for der_i in der:
                    if (der_iter == 0):
                        grad_i = jax.vmap(jax.jacfwd(self.networkArrayReg, int(der_i)), (0, 0, None, None, None, None), 0)
                    else:
                        grad_i = jax.vmap(jax.jacfwd(grad_i, int(der_i)), (0, 0, None, None, None, None), 0)
                    der_iter += 1
                der_print = self.parseDerivativeString(der)
                # value of its derivatives
                NNoutput[field]["interior_" + der_print] = jnp.squeeze(grad_i(xTrain, yTrain, params_i[field], scale, numWaves, opt_args))

            # compute corresponding derivatives on the spatial boundary
            if self.hasSpatialBoundary[field]:
                NNoutput[field]["boundary_value"] = jnp.squeeze(jax.vmap(self.networkArrayReg, (0, 0, None, None, None, None),  0)
                                                                (xBdryTrain, yBdryTrain, params_i[field], scale, numWaves, opt_args))
            ...
            for der in self.bdryDerivatives[field]:
                ...
                for der_i in der:
                    ...
                    der_iter += 1
                der_print = self.parseDerivativeString(der)
                NNoutput[field]["boundary_" + der_print] = jnp.squeeze(grad_i(xBdryTrain, yBdryTrain, params_i[field], scale, numWaves, opt_args))
    ...
    @partial(jax.jit, static_argnums=(0,2,14,15,))
    def computeLoss(self, params, k, 
        ...
        for field in self.solution_fields:
            ...
            phi[field]["interior_value"] = self.createNetwork(X, Y, params_i[field], activationcoeffs[field], scalek, 
            ...
            for der in self.intDerivatives[field]:
                ...
                for der_i in der:
                    ...
                    der_iter += 1
                der_print = self.parseDerivativeString(der)
                phi[field]["interior_" + der_print] = (jax.vmap(grad_i, (0, 0, None, None, None, None, None, None))
                                                       (X, Y, params_i[field], activationcoeffs[field], scalek, numWaves, opt_args["alphaSize"], opt_args["coeffs"]))

            # compute required boundary fields 
            if self.hasSpatialBoundary[field]:
                phi[field]["boundary_value"] = self.createNetwork(Xbdry, Ybdry, params_i[field], activationcoeffs[field], scalek, 
                                                                  numWaves, opt_args["alphaSize"], opt_args["coeffs"])
            ...
            for der in self.bdryDerivatives[field]:
                ...
                for der_i in der:
                    ...
                    der_iter += 1
                der_print = self.parseDerivativeString(der)
                phi[field]["boundary_" + der_print] = (jax.vmap(grad_i, (0, 0, None, None, None, None, None, None))
                                                       (Xbdry, Ybdry, params_i[field], activationcoeffs[field], scalek, numWaves, opt_args["alphaSize"], opt_args["coeffs"]))

        # compute loss function
        if (self.isExtendedNetwork):
            resWeight = jnp.power(jnp.sqrt(jnp.pow(X, 2) + jnp.pow(Y, 2)), self.beta)
            # resWeightBdry = jnp.power(jnp.sqrt(jnp.pow(Xbdry, 2) + jnp.pow(Ybdry, 2)), self.beta)
            resWeightBdry = 1.0
        else:
            resWeight = jnp.power(jnp.sqrt(jnp.pow(X, 2) + jnp.pow(Y, 2)), self.beta)
            resWeightBdry = 1.0
        ...
        normint1 = jnp.sum(jnp.multiply(W, jnp.square(jnp.multiply(resWeight, phi["u"]["interior_xx"] + phi["u"]["interior_yy"]))))
        normbdry1 = jnp.multiply(self.boundaryPen1, jnp.sum(jnp.multiply(Wbdry, jnp.square(resWeightBdry*phi["u"]["boundary_value"]))))
        normbdry2 = jnp.multiply(self.boundaryPen2, jnp.sum(jnp.multiply(Wbdry, jnp.square(resWeightBdry*(jnp.multiply(TX, phi["u"]["boundary_x"]) + jnp.multiply(TY, phi["u"]["boundary_y"]))))))
        norm = jnp.sqrt(normint1 + normbdry1 + normbdry2)
        
        # compute the value of a(u0,phi)
        r1 = jnp.sum(jnp.multiply(W, jnp.multiply(jnp.multiply(resWeight, SOL0["u"]["interior_xx"][:,0] + SOL0["u"]["interior_yy"][:,0]), 
                                                  jnp.multiply(resWeight, phi["u"]["interior_xx"] + phi["u"]["interior_yy"]))))
        ...
        r3 = jnp.multiply(self.boundaryPen2, jnp.sum(jnp.multiply(Wbdry, jnp.multiply(resWeightBdry*(jnp.multiply(TX, SOL0["u"]["boundary_x"][:,0]) + jnp.multiply(TY, SOL0["u"]["boundary_y"][:,0])),
                                                                                      resWeightBdry*(jnp.multiply(TX, phi["u"]["boundary_x"]) + jnp.multiply(TY, phi["u"]["boundary_y"]))))))

        # compute the value of L(phi)
        r4 = jnp.sum(jnp.multiply(W, jnp.multiply(jnp.multiply(resWeight, F1), jnp.multiply(resWeight, -phi["u"]["interior_xx"] - phi["u"]["interior_yy"]))))
        r5 = jnp.multiply(self.boundaryPen1, jnp.sum(jnp.multiply(Wbdry, jnp.multiply(resWeightBdry*U0bdryexact["value"], resWeightBdry*phi["u"]["boundary_value"]))))
        r6 = jnp.multiply(self.boundaryPen2, jnp.sum(jnp.multiply(Wbdry, jnp.multiply(resWeightBdry*(jnp.multiply(TX, U0bdryexact["x"]) + jnp.multiply(TY, U0bdryexact["y"])),
                                                                                      resWeightBdry*(jnp.multiply(TX, phi["u"]["boundary_x"]) + jnp.multiply(TY, phi["u"]["boundary_y"]))))))
    ...
    def computeError(self, solution0):
        ...
        if (self.isExtendedNetwork):
            resWeight = np.sqrt(xVal**2 + yVal**2)**self.beta
            # resWeightBdry = np.sqrt(xBdryVal**2 + yBdryVal**2)**self.beta
            resWeightBdry = 1.0
        else:
            resWeight = np.sqrt(xVal**2 + yVal**2)**self.beta
            resWeightBdry = 1.0

        uexact = self.exactSol(xVal, yVal)
        ...
        ubdryexact = self.exactSol(xBdryVal, yBdryVal)
        uxbdryexact, uybdryexact = self.exactDx(xBdryVal, yBdryVal)

        exact1 = f
        exact2 = self.QuadRulesVal.boundary_t_x * uxbdryexact + self.QuadRulesVal.boundary_t_y * uybdryexact
        # print(np.average(np.abs(resWeight*(exact1-f1))), np.average(np.abs(resWeight*(exact2-f2))), np.average(np.abs(exact3)), np.average(np.abs(resWeight*exact4)), np.average(np.abs(resWeight*exact5)))

        approx1 = -solution0["u"]["interior_xx"] - solution0["u"]["interior_yy"]
        approx2 = self.QuadRulesVal.boundary_t_x * solution0["u"]["boundary_x"] + self.QuadRulesVal.boundary_t_y * solution0["u"]["boundary_y"]
        ...
        Energy_k += self.boundaryPen2 * np.sum(self.QuadRulesVal.boundary_w * (exact2 - approx2)**2)
        ...
        Energy0 += self.boundaryPen2 * np.sum(self.QuadRulesVal.boundary_w * (resWeightBdry*exact2)**2)
    ...
    def appendBasis(self, k, params, activationcoeffs, activationcoeffs_singular, scale, numWaves, alphaSize):
        ...
        for field in self.solution_fields:
            ...
            field_basis["interior_value"] = self.createNetwork(xTrain, yTrain, params_i[field], activationcoeffs[field], scale, 
                                                               numWaves, opt_args["alphaSize"], opt_args["coeffs"])
            for der in self.intDerivatives[field]:			
                # compute the requested derivative
                der_iter = 0
                for der_i in der:
                    ...
                    der_iter += 1
                der_print = self.parseDerivativeString(der)
                field_basis["interior_" + der_print] = (jax.vmap(grad_i, (0, 0, None, None, None, None, None, None))
                                                        (xTrain[:,0], yTrain[:,0], params_i[field], activationcoeffs[field], scale, numWaves, opt_args["alphaSize"], opt_args["coeffs"]))

            # spatial boundary value and derivatives
            if self.hasSpatialBoundary[field]:
                field_basis["boundary_value"] = self.createNetwork(xBdryTrain, yBdryTrain, params_i[field], activationcoeffs[field], scale, 
                                                                   numWaves, opt_args["alphaSize"], opt_args["coeffs"])
            for der in self.bdryDerivatives[field]:			
                # compute the requested derivative
                der_iter = 0
                for der_i in der:
                    ...
                    der_iter += 1
                der_print = self.parseDerivativeString(der)
                field_basis["boundary_" + der_print] = (jax.vmap(grad_i, (0, 0, None, None, None, None, None, None))
                                                        (xBdryTrain[:,0], yBdryTrain[:,0], params_i[field], activationcoeffs[field], scale, numWaves, opt_args["alphaSize"], opt_args["coeffs"]))
        ...
        for field in self.solution_fields:
            ...
            # interior value and derivatives
            field_basis["interior_value"] = self.createNetwork(xVal, yVal, params_i[field], activationcoeffs[field], scale, 
                                                               numWaves, opt_args["alphaSize"], opt_args["coeffs"])
            for der in self.intDerivatives[field]:			
                # compute the requested derivative
                der_iter = 0
                ...
                der_print = self.parseDerivativeString(der)
                field_basis["interior_" + der_print] = (jax.vmap(grad_i, (0, 0, None, None, None, None, None, None))
                                                        (xVal[:,0], yVal[:,0], params_i[field], activationcoeffs[field], scale, numWaves, opt_args["alphaSize"], opt_args["coeffs"]))

            # spatial boundary value and derivatives
            if self.hasSpatialBoundary[field]:
                field_basis["boundary_value"] = self.createNetwork(xBdryVal, yBdryVal, params_i[field], activationcoeffs[field], scale, 
                                                                   numWaves, opt_args["alphaSize"], opt_args["coeffs"])
            for der in self.bdryDerivatives[field]:			
                # compute the requested derivative
                der_iter = 0
                ...
                der_print = self.parseDerivativeString(der)
                field_basis["boundary_" + der_print] = (jax.vmap(grad_i, (0, 0, None, None, None, None, None, None))
                                                        (xBdryVal[:,0], yBdryVal[:,0], params_i[field], activationcoeffs[field], scale, numWaves, opt_args["alphaSize"], opt_args["coeffs"]))
        ...
        for field in self.solution_fields:
            ...
            field_basis["interior_value"] = self.createNetwork(xStream, yStream, params_i[field], activationcoeffs[field], scale, 
                                                               numWaves, opt_args["alphaSize"], opt_args["coeffs"])
    ...
    def galerkinSolve(self, k):
        ...
        f1Train = self.sourceFunc(xTrain, yTrain)
        G1  = self.exactSol(xBdryTrain, yBdryTrain)
        G1x, G1y = self.exactDx(xBdryTrain, yBdryTrain)
        ...
        if (self.isExtendedNetwork):
            w = np.squeeze(np.sqrt(xTrain**2 + yTrain**2)**self.beta)
            # wBdry = np.squeeze(np.sqrt(xBdryTrain**2 + yBdryTrain**2)**self.beta)
            wBdry = 1.0
        else:
            w = np.squeeze(np.sqrt(xTrain**2 + yTrain**2)**self.beta)
            wBdry = 1.0
        ...
        for idx in range(k+1):
            for jdx in range(k+1):
                A[idx,jdx] = np.sum(wGlobalFlattened * (w*(self.basis["u"][idx]["interior_xx"] + self.basis["u"][idx]["interior_yy"])) * 
                                                       (w*(self.basis["u"][jdx]["interior_xx"] + self.basis["u"][jdx]["interior_yy"])))
                ...
                A[idx,jdx] += self.boundaryPen2 * np.sum(wBdryFlattened * (wBdry*(txFlattened * self.basis["u"][idx]["boundary_x"] + tyFlattened * self.basis["u"][idx]["boundary_y"])) *
                                                                          (wBdry*(txFlattened * self.basis["u"][jdx]["boundary_x"] + tyFlattened * self.basis["u"][jdx]["boundary_y"])))
            F[idx] = np.sum(wGlobalFlattened * (w*(-self.basis["u"][idx]["interior_xx"] - self.basis["u"][idx]["interior_yy"])) * 
                                                 (w*f1Train))
            F[idx] += self.boundaryPen1 * np.sum(wBdryFlattened * (wBdry*self.basis["u"][idx]["boundary_value"]) * (wBdry*G1))
            F[idx] += self.boundaryPen2 * np.sum(wBdryFlattened * (wBdry*(txFlattened * self.basis["u"][idx]["boundary_x"] + tyFlattened * self.basis["u"][idx]["boundary_y"])) *
    ...
    def plotSurface(self, k, x, y, z, myTitle, myPath):
        ...
        ng = int(np.sqrt(x.shape[0]))
        x1 = np.reshape(x[0:4*ng**2,0], [ng,ng])
        y1 = np.reshape(y[0:4*ng**2,0], [ng,ng])
        z1 = np.reshape(z[0:4*ng**2,0], [ng,ng])
    ...
    def generateBasis(self):
        ...
        f1Train = self.sourceFunc(xTrain, yTrain)
        f1Val = self.sourceFunc(xVal, yVal)
        ...
        G1Train = self.exactSol(xBdryTrain, yBdryTrain)
        G1xTrain, G1yTrain = self.exactDx(xBdryTrain, yBdryTrain)
        UbdryExactTrain = {}
        UbdryExactTrain["value"] = np.reshape(G1Train, [xBdryTrain.shape[0],])
        UbdryExactTrain["x"] = np.reshape(G1xTrain, [xBdryTrain.shape[0],])
        UbdryExactTrain["y"] = np.reshape(G1yTrain, [xBdryTrain.shape[0],])

        G1Val = self.exactSol(xBdryVal, yBdryVal)
        G1xVal, G1yVal = self.exactDx(xBdryVal, yBdryVal)
        UbdryExactVal = {}
        UbdryExactVal["value"] = np.reshape(G1Val, [xBdryVal.shape[0],])
        UbdryExactVal["x"] = np.reshape(G1xVal, [xBdryVal.shape[0],])
        UbdryExactVal["y"] = np.reshape(G1yVal, [xBdryVal.shape[0],])
        ...
        while (k < self.maxRef):
            ...
            while (np.abs(gradCheck0 - gradCheck1)/np.abs(gradCheck0) > self.gradTol and i < self.maxEpoch):
                ...
                _, _, _, _, wxm, wym = params[0]

                print("Iter: ", k, ", Epoch: ", i, ", Loss: ", loss_i, ", Energy: ", Energy_k, ", L2 velocity: ", L2_k, ", Waves: ", wxm, wym)
            ...
            # error 
            ueStream = self.exactSol(xStream, yStream)
            ...
            self.plotSurface(k, xStream, yStream, ERROR_U, myTitle, myPath)

            myTitle = r"Approximate error $\varphi_{i}^{NN}$"
            myPath = self.RESULTS_PATH + "/basis_" + str(k) + ".png"
            self.plotSurface(k, xStream, yStream, np.reshape(BASIS_PHI, [xStream.shape[0],1]), myTitle, myPath)
            ...
            self.plotSurface(k, xStream, yStream, np.reshape(uStream["u"]["interior_value"], [xStream.shape[0],1]), myTitle, myPath)
            ...
            mystr = 'Error vs. Number of Basis Functions'
        ...
        pickle.dump(mdic, open(self.RESULTS_PATH + "/saved_poisson2D_results.pkl", "wb"))
