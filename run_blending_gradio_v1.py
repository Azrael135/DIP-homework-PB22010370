...
def cal_laplacian_loss(foreground_img, blended_img, background_mask):
    ...
    blend_lap = F.conv2d(blended_img, lap_kernel, padding=1, groups=3)

    mask = background_mask.expand_as(fg_lap)
    loss = ((fg_lap - blend_lap) ** 2 * mask).sum() / (mask.sum() + 1e-8)
...
def blending(foreground_image_original, background_image_original, dx, dy, polygon_state):
    ...
    # ===== construct a foreground canvas in background coordinates =====
    H_bg, W_bg = background_np.shape[:2]
    H_fg, W_fg = foreground_np.shape[:2]

    fg_canvas = np.zeros_like(background_np, dtype=np.uint8)
    fg_canvas_mask = np.zeros((H_bg, W_bg), dtype=np.uint8)
    ...
    for y in range(H_fg):
        for x in range(W_fg):
            ny = y + shift_y
            nx = x + shift_x
            if 0 <= ny < H_bg and 0 <= nx < W_bg:
                if foreground_mask[y, x] > 0:
                    fg_canvas[ny, nx] = foreground_np[y, x]
                    fg_canvas_mask[ny, nx] = 255

    fg_img_tensor = torch.from_numpy(fg_canvas).to(device).permute(2, 0, 1).unsqueeze(0).float() / 255.
    fg_mask_tensor = torch.from_numpy(fg_canvas_mask).to(device).unsqueeze(0).unsqueeze(0).float() / 255.
    ...
    blended_img = bg_img_tensor.clone()
    mask_expanded = bg_mask_tensor.bool().expand(-1, 3, -1, -1)
    blended_img[mask_expanded] = fg_img_tensor[mask_expanded]
    blended_img = blended_img.clone().detach().requires_grad_(True)

    optimizer = torch.optim.Adam([blended_img], lr=5e-3)

    iter_num = 5000
    for step in range(iter_num):
        blended_img_for_loss = bg_img_tensor * (1. - bg_mask_tensor) + blended_img * bg_mask_tensor

        lap_loss = cal_laplacian_loss(fg_img_tensor, blended_img_for_loss, bg_mask_tensor)

        rgb_mask = bg_mask_tensor.expand_as(blended_img_for_loss)
        rgb_loss = ((fg_img_tensor - blended_img_for_loss) ** 2 * rgb_mask).sum() / (rgb_mask.sum() + 1e-8)

        loss = lap_loss + 0.1 * rgb_loss
        ...
        if step % 100 == 0:
            print(f"Step {step}/{iter_num}, total={loss.item():.6f}, lap={lap_loss.item():.6f}, rgb={rgb_loss.item():.6f}")

        if step == int(iter_num * 0.5):
            optimizer.param_groups[0]['lr'] *= 0.2
        if step == int(iter_num * 0.8):
            optimizer.param_groups[0]['lr'] *= 0.2
...
demo.launch(server_name="127.0.0.1", server_port=8889, inbrowser=True)